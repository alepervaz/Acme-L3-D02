...
<acme:form>
	...
	<jstl:choose>
		<jstl:when test="${acme:anyOf(_command, 'show|update|delete') && isAuditor}">
			...
			<acme:submit test="${myAudit && draftMode}" code="authenticated.audit.form.button.update" action="/auditor/audit/update"/>
			<acme:submit test="${myAudit && draftMode}" code="authenticated.audit.form.button.delete" action="/auditor/audit/delete"/>
		</jstl:when>
		<jstl:when test="${_command == 'create'}">
			<acme:submit code="authenticated.audit.form.button.create" action="/auditor/audit/create"/>
		</jstl:when>		
	</jstl:choose>	
